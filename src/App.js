...
class App extends Component {
  ...
  tick = () => {
    ...
    if (time > 0) {
      ...
      this.setState({scoreCardDisplay: true})
    }
  }
  ...
  onPlayGame = thumbnailUrl5 => {
    const {mainImageUrl, score} = this.state
    const score2 = score
    const MainImgItem = imagesList.find(eachItem => {
      if (eachItem.imageUrl === mainImageUrl) {
        return true
      }
      return false
    })
    console.log(MainImgItem)
    const mainImgID = MainImgItem.thumbnailUrl
    if (mainImgID === thumbnailUrl5) {
      this.setState(previousState => ({
        score: previousState.score + 1,
        mainImageUrl: imagesList[Math.ceil(Math.random() * 10)].imageUrl,
      }))
    } else {
      this.setState({score: score2, scoreCardDisplay: true})
    }
  }
  ...
  componentWillUmount() {
    this.setState({scoreCardDisplay: true, time: 0})
  }
  ...
  render() {
    ...
    const filterdImagesList = initialImagesList.filter(
          ...
          <div className="nav-container">
            ...
            <div className="score-container">
              ...
              <img
                ...
                alt="timer"
              />
              <p className="score-tag">{time} sec</p>
            </div>
          </div>
        </nav>
        {!scoreCardDisplay && (
          <div className="container-1">
            <img src={mainImageUrl} className="main-image" alt="match" />
            ...
            <ul className="tabs-items-container">
              {filterdImagesList.map(eachItem => (
                <TabItems
                  details={eachItem}
                  key={eachItem.id}
        ...
        {scoreCardDisplay && (
          <div className="score-card">
            <img
              ...
              alt="trophy"
            />
            <h1 className="score-result-title">Your Score</h1>
            <p className="score">{score}</p>
            <button
              className="play-again-button"
              type="button"
              onClick={this.onButtonPlayAgain}
            >
              <img
